...
impl<R: Ring> Add for &VirtualPolynomial<R> {
    ...
    fn add(self, other: &VirtualPolynomial<R>) -> Self::Output {
        ...
        for products in other.products.iter() {
            let cur: Vec<Arc<DenseMultilinearExtension<R>>> = products.1
                .iter()
                .map(|&x| other.flattened_ml_extensions[x].clone())
                .collect();

            res.add_mle_list(cur, products.0).expect("add product failed");
        }
        end_timer!(start);
        res
    }
...
impl<R: Ring> VirtualPolynomial<R> {
    ...
    pub fn add_mle_list(
        ...
        if mle_list.is_empty() {
            return Err(ArithErrors::InvalidParameters("input mle_list is empty".to_string()));
        }
    ...
    pub fn mul_by_mle(
        ...
        let mle_index = match self.raw_pointers_lookup_table.get(&mle_ptr) {
            Some(&p) => p,
            None => {
                self.raw_pointers_lookup_table.insert(mle_ptr, self.flattened_ml_extensions.len());
    ...
    pub fn evaluate(&self, point: &[R]) -> Result<R, ArithErrors> {
        ...
        let evals: Vec<R> = self.flattened_ml_extensions
            .iter()
            .map(|x| {
    ...
    pub fn rand<Rn: RngCore>(
        ...
        for _ in 0..num_products {
            let num_multiplicands = rng.gen_range(
                num_multiplicands_range.0..num_multiplicands_range.1
            );
...
pub fn eq_eval<R: Ring>(x: &[R], y: &[R]) -> Result<R, ArithErrors> {
    if x.len() != y.len() {
        return Err(ArithErrors::InvalidParameters("x and y have different length".to_string()));
    }
