...
pub fn identity_permutation_mles<R: Ring>(
    ...
    for i in 0..num_chunks {
        ...
        res.push(Arc::new(DenseMultilinearExtension::from_evaluations_vec(num_vars, s_id_vec)));
    }
    res
}
...
pub fn fix_variables<R: Ring>(
    ...
    assert!(partial_point.len() <= poly.num_vars, "invalid size of partial point");
...
pub fn merge_polynomials<R: Ring>(
    ...
    for poly in polynomials.iter() {
        if nv != poly.num_vars() {
            return Err(
                ArithErrors::InvalidParameters("num_vars do not match for polynomials".to_string())
            );
        }
    }
    ...
    Ok(Arc::new(DenseMultilinearExtension::from_evaluations_vec(merged_nv, scalars)))
}
