...
impl<R: Ring> MultilinearExtension<R> for DenseMultilinearExtension<R> {
    ...
    fn fix_variables(&self, partial_point: &[R]) -> Self {
        assert!(partial_point.len() <= self.num_vars, "too many partial points");
...
impl<R: Ring> Zero for DenseMultilinearExtension<R> {
    fn zero() -> Self {
        Self { num_vars: 0, evaluations: vec![R::zero()] }
    }
...
impl<R: Ring> AddAssign<(R, &DenseMultilinearExtension<R>)>
    for DenseMultilinearExtension<R>
    where R: Copy + std::ops::AddAssign
{
