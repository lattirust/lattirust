...
use crate::lova::util::{Instance, PublicParameters, SECPARAM};
use crate::nimue::merlin::SerMerlin;
use crate::nimue::traits::ChallengeFromRandomBytes;

pub fn verify_folding<F: ConvertibleField>(merlin: &mut Merlin, crs: &PublicParameters<F>, instance_1: &Instance<F>, instance_2: &Instance<F>) -> Result<Instance<F>, ProofError> {
    let committed_decomp_witness = merlin.next_matrix(crs.commitment_mat.nrows(), 2 * SECPARAM * crs.decomposition_length)?;

    let inner_products = merlin.next_symmetric_matrix_ser::<i128>(2 * SECPARAM)?.into();
    ...
    check_eq!(recompose_matrix(&committed_decomp_witness, &crs.powers_of_basis().as_slice()), commitment);
