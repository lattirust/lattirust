        ...
        Self: ByteChallenges
{
    ...
    fn challenge_vec<T, A: FromRandomBytes<T>>(&mut self, size: usize) -> Result<Vec<T>, IOPatternError> {
    ...
    fn challenge_vector<T: Scalar, A: FromRandomBytes<T>>(&mut self, size: usize) -> Result<Vector<T>, IOPatternError> {
        self.challenge_vec::<T, A>(size).map(|v| Vector::<T>::from_vec(v))
    }

    fn challenge_vectors<T: Scalar, A: FromRandomBytes<T>>(&mut self, size: usize, n_vectors: usize) -> Result<Vec<Vector<T>>, IOPatternError> {
    ...
    fn challenge_matrix<T: Scalar, A: FromRandomBytes<T>>(&mut self, n_rows: usize, n_cols: usize) -> Result<Matrix<T>, IOPatternError> {
        self.challenge_vec::<T, A>(n_rows * n_cols).map(|v| Matrix::<T>::from_vec(n_rows, n_cols, v))
    }

    fn challenge_matrices<T: Scalar, A: FromRandomBytes<T>>(&mut self, n_rows: usize, n_cols: usize, n_matrices: usize) -> Result<Vec<Matrix<T>>, IOPatternError> {
